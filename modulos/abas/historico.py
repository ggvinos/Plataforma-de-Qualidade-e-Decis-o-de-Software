...
def aba_historico(df: pd.DataFrame):
    ...
    st.warning("**Dados demonstrativos** — Os gráficos desta aba são gerados com valores fictícios para ilustrar o potencial da ferramenta. Não utilize estas informações para tomada de decisão.")
    ...
    with col2:
        if ultimo_fpy >= 80:
            ...
            cor = "#f59e0b"
            status = f" {80 - ultimo_fpy:.1f}% abaixo"
    ...
    with col3:
        if ultimo_lead <= 7:
            ...
            cor = "#ef4444"
            status = f" Meta: ≤7d"
        st.markdown(mini_card(f"{ultimo_lead:.1f}d", " Lead Time", status, cor), unsafe_allow_html=True)
    ...
    with st.expander(" Lead Time Médio", expanded=False):
